...
object GlintCustomizeCommand {
    private fun getCurrentItemId(): String {
        if (!Utils.inSkyblock) throw IllegalArgumentException("§cYou must be in Skyblock to use this command!")
        val item: ItemStack = mc.player?.mainHandStack ?: throw IllegalArgumentException("§cYou need to hold an item that you wish to customize!")
        return ItemUtil.getSkyBlockItemID(item) ?: throw IllegalArgumentException("§cThat isn't a valid SkyBlock item (missing SB ID)!")
    }
    ...
    fun colorSet(
        ...
        try {
            ...
            throw IllegalArgumentException("$failPrefix §cUnable to get a color from '$colorString'. Use hex (e.g. #FF00AA) or color names.")
        }
    }
