...
object HollowWaypointCommand {
    ...
    fun setWaypoint(
        ...
        if (loc != null) {
            loc.locX = (mc.player!!.x - 200).coerceIn(0.0, 624.0)
            loc.locY = mc.player!!.y
            loc.locZ = (mc.player!!.z - 200).coerceIn(0.0, 624.0)
            UChat.chat("$successPrefix §aSuccessfully set location waypoint $name to your current location.")
        } else {
            CHWaypoints.waypoints[name] = mc.player!!.blockPos
    ...
    fun setWaypoint(
        ...
        if (loc != null) {
            ...
            CHWaypoints.waypoints[name] = BlockPos(MathHelper.floor(x), MathHelper.floor(y), MathHelper.floor(z))
