...
public abstract class MixinEntityFishHook {
    ...
    private boolean allowLavaBobber(FluidState instance, TagKey<Fluid> tag, Operation<Boolean> original) {
        boolean orig = original.call(instance, tag);
        if (!Utils.INSTANCE.getInSkyblock() || !Skytils.getConfig().getLavaBobber()) return orig;
        return orig || instance.isIn(FluidTags.LAVA);
    }
}
